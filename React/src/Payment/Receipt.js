...
function Receipt() {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:8080/api/Payment/GetByStudentId/${student_id}`)
      .then((response) => response.json())
      .then((data) => {
        setStudentData(data);

        // Fetch course data using studentData.course_id
        fetch(`http://localhost:8080/api/courses/getById/${data.course_id}`)
    ...
    fetch(`http://localhost:8080/api/Payment/GetbyStudent/${student_id}`)
      .then((response) => response.json())
      .then((data) => {
        setPaymentData(data);
